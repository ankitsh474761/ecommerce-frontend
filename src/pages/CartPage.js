...
const CartPage = () => {
  ...
  const handlePayment = async () => {
    try {
      ...
      const res = await fetch("http://localhost:4000/api/v1/products/order", {
        method: "POST",
        headers: {
      ...
      const options = {
        ...
        handler: async function (response) {
         ...
         const validateRes = await fetch(
           "http://localhost:4000/api/v1/products/order/validate",
           {
             method: "POST",
            ...
            if(success){
              const addDatatoDb = await fetch(
                "http://localhost:4000/api/v1/products/orderDone",{
                  method:"POST",
                  body:JSON.stringify({
                    email,
                   cart 
    ...
    <Layout>
        <div className="cart-page" style={{height:"100%"}}>
          <div className="products1">
            ...
            <h1 style={{textAlign:"center","fontSize":"40px",color:"white"}}>{`Hello, ${auth?.token && auth?.user?.name}`}!</h1>
          ...
          <div className="products">
            ...
            <div className="cart-summary">
              <h2 style={{textAlign:"center",fontSize:"30px"}}>Cart Summary</h2>
                ...
                <>
                  <div style={{marginTop:"20px"}}>
                    {auth?.token ? (
                      <>
                        <button
                          onClick={() => navigate("/dashboard/user/profile")} className='address'
                        >
                          Update Address
                        </button>
                      </>
                      ...
                      <>
                        <button className='CartBtn'
                          onClick={() =>
                            ...
                            style={{"padding":"10px","borderRadius":"5px","border":"none","backgroundColor":"red","fontWeight":"500","fontSize":"16px","color":"white","cursor":"pointer",marginRight:"120px"}}
                        >
                           Login to Checkout
                        </button>
                      </>
              ...
              <div style={{marginTop:"20px"}}>
                {auth?.user ? (
                  <button onClick={handlePayment} className='cartBtn'>Make Payment</button>
                ) : (
                  <></>
                )}
              </div>
